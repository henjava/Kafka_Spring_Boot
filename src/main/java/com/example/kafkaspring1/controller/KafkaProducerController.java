...
public class KafkaProducerController
{
    ...
    {
        temp.send("new",message);
    }

   @Bean
  public NewTopic creatTopic()
  {
      return new NewTopic("new",3,(short) 1);
  }
    ...
    @KafkaListener(groupId = "group_id", topics = "new", containerFactory = "kafkaListenerContainerFactory")
    public List<Map<String, String>> getMsgFromTopic(RandoMessage data) {
        for (int i=0; i< data.getRandoMessage().size(); i++) {//check if its num to string
